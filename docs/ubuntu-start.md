...
Die Config passt dafür schon: `net.host = 127.0.0.1`, `net.port = 8765`,
`net.token = invchecker-change-me` – und die Mod hat genau dieselben Werte als
Default (`botHost = 127.0.0.1`, `botPort = 8765`, gleiches Token). Du musst also
...
```
[ ok  ] TCP für Mod bereit auf 127.0.0.1:8765 (Token: STANDARDWERT – bitte ändern!)
...
| Mod zeigt `verbinde erneut …` dauerhaft | Läuft der Bot? `ss -tlnp \| grep 8765` muss `LISTEN 127.0.0.1:8765` zeigen |
